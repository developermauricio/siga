...
class Settings extends DokanSettings {
    ...
    public function load_settings_header( $header, $query_vars ) {
        ...
        if ( $query_vars == 'shipping' ) {
            ...
            // TODO: cambios $header = sprintf( '%s <span style="position:absolute; right:0px;"><a href="%s" class="dokan-btn dokan-btn-default"><i class="fa fa-gear"></i> %s</a></span>', __( 'Shipping Settings', 'dokan' ), $settings_url, __( 'Click here to add Shipping Policies', 'dokan' ) );
            $header = sprintf( '%s <span style="position:absolute; right:0px;"><a href="%s" class="dokan-btn dokan-btn-default"><i class="fa fa-gear"></i> %s</a></span>', __( 'Shipping Settings', 'dokan' ), $settings_url, __( 'Haga clic aquí para agregar políticas de envío', 'dokan' ) );
        }
    ...
    public function load_settings_helper_text( $help_text, $query_vars ) {
        ...
        if ( $query_vars == 'shipping' ) {
            ...
            $help_text = sprintf ( '<p>%s</p>',
                __( 'Una zona de envío es una región geográfica donde se ofrece un determinado conjunto de métodos de envío. Emparejaremos a un cliente con una sola zona usando su dirección de envío y le presentaremos los métodos de envío dentro de esa zona.', 'dokan' ),
                __( 'Si desea utilizar el sistema de envío anterior', 'dokan' ),
                esc_url( dokan_get_navigation_url('settings/regular-shipping' ) ),
                __( 'Click Here', 'dokan' )
            );
            /* TODO: cambios $help_text = sprintf ( '<p>%s</p>',
    ...
    **/
    ...
    public function render_biography_form( $vendor_id, $store_info ) {
        ...
        <div class="dokan-form-group">
            ...
            <label class="dokan-w3 dokan-control-label"><?php _e( 'Biografía', 'dokan' ); ?></label>
